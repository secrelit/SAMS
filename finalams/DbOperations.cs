  ...
  //  }
    public class DbOperations
    {
        ...
        public void InsertToDailyPunchTime(PunchTimeDetails ptd, string name, finalams.Date date)
        {
            ...
            string str = "insert into daily_punch_time values('" + name + "'," + date.Day + "," + date.Month + "," + date.Year + "," + ptd.InTime.Hour + "," + ptd.InTime.Minute + "," + ptd.OutTime.Hour + "," + ptd.OutTime.Minute + ",'" + ptd.DateCategory + "','" + ptd.Remark + "')";
            cmd = new MySqlCommand(str, con);
            cmd.ExecuteNonQuery();
            Console.WriteLine("values inserted");
            con.Close();
        ...
        public Boolean IsNewStaff(string name)
        {
            ...
            con = new MySqlConnection("server=localhost;User Id=root;password=flatron;database=secrelit_sams;Persist Security Info=True");
            con.Open();
